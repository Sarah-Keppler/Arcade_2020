...
namespace Game
{
    class Menu : protected AGame
    {
    public:
	Menu() noexcept;
	~Menu() = default;

	void load(std::unique_ptr<Graph::IGraph> &lib) noexcept;
	void leave() noexcept;
	//Arcade::Type getType() const noexcept;
	//std::string getName() const noexcept;

	//Arcade::Keywords play(); May change for the three next functions
	Arcade::Keywords handleEvents();
	void handleUpdate(float elapsedTIme);
	void display();
    private:
	/*
	...
	*/
	 ...
	 */
	virtual void load(std::unique_ptr<Graph::IGraph> lib);
